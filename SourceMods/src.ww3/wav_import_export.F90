...
contains
  ...
  subroutine advertise_fields(importState, ExportState, flds_scalar_name, rc)
    ...
    ! Special field with lab sea masked
    call fldlist_add(fldsToWav_num, fldsToWav, 'Si_ifrac_wav' )
...
!      end if
      if (state_fldchk(importState, 'Si_ifrac_wav')) then
        call SetGlobalInput(importState, 'Si_ifrac_wav', vm, global_data, rc)
        if (ChkErr(rc,__LINE__,u_FILE_u)) return
        call FillGlobalInput(global_data, ICEI)
      end if
    end if
