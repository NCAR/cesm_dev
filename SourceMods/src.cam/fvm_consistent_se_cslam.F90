...
module fvm_consistent_se_cslam
  ...
  public :: cslam_q_filter_geom_init  ! called from dyn_grid after fvm_init3
contains
  !
  !**************************************************************************************
  !
  ...
  subroutine run_consistent_se_cslam(elem,fvm,hybrid,dt_fvm,tl,nets,nete,hvcoord,&
    ...
    if (cslam_q_filter) then
      !
      ! Mass-conservative del4 filter on water vapor on the CSLAM grid.
      ! Keep del4_cslam_qgll = .false. while active (no double damping).
      !
...
!          if (mass_flux_se(i,j,iside)>eps) then
  ...
  ! nu4 = cslam_q_filter_mult * nu_p, constant in the vertical.
  !
  ...
  ! Stability: dt_sub*mult*nu_p*(8/A_min)^2 < 2 (2D checkerboard bound);
  ...
  subroutine apply_cslam_q_filter_del4(fvm, hybrid, nets, nete, kmin, kmax, dt_fvm, limiter, xdiff)
    ...
    use dimensions_mod , only: cslam_q_filter_mult
    ...
    real (kind=r8)            :: nu4k               ! mult*nu_p / rearth^4 (constant per level)
    ...
    do ie = nets, nete
       ...
       do k = kmin, kmax
          nu4k = cslam_q_filter_mult * nu_p * rearth4_inv   ! constant in the vertical
          if (xdiff) then
  ...
  !
  subroutine cslam_q_filter_geom_init(elem, fvm, nets, nete)
    implicit none
    type (element_t) , intent(in)    :: elem(:)
    type (fvm_struct), intent(inout) :: fvm(:)
