...
subroutine zm_convr_init(plev, plevp, cpair, epsilo, gravit, latvap, tmelt, rair, &
                    ...
                    zmconv_parcel_pbl, zmconv_parcel_hscale,&
...
subroutine zm_convr_run(     ncol    ,pver    , &
                    ...
                    ql      ,rliq    ,landfrac, &
                    buoy    ,wm_incld ,w_incld ,lcl ,pl ,tl, hmax ,maxi , &
                    lel     ,plev_ke, dmpdz, tke, &
                    rice    ,lengath ,scheme_name, errmsg  ,errflg, iulog)
...
!
   ...
   real(kind_phys), intent(in) :: omega(:,:)  ! RBN - Omega to be used in parcel energy calculation   (ncol,pver)
...
!
   ...
   do i = 1,ncol
      ...
      wm_incld(i) = 0._kind_phys  !RBN: Initialize in-cloud parcel velocity
...
!
   ...
   call buoyan_dilute(  ncol   ,pver     , &
               ...
               rgas    ,grav    ,cpres   ,msg     , &
               zi      ,zs      ,tpert   , landfrac, wm_incld, &              
               buoy    ,w_incld ,pl      ,hmax   ,plev_ke, dmpdz, tke, &
               lat,    long,  errmsg  ,errflg, iulog)
...
!
   ...
   real(kind_phys), intent(in) :: omega(ncol,pver) ! RBN - Omega to be used in parcel energy calculation  
...
!!! tv=tv : tpv=tpv : qstp=q : buoy=0.
   ...
   plev_ke(:ncol,:) = 0._kind_phys
   w_incld(:ncol,:) = 0._kind_phys
   w_nrg(:ncol,:) = -rgas*t(:ncol,:)*omega(:ncol,:)/(grav*100._kind_phys*p(:ncol,:))  
...
!!!   RBN 9/9/04   !!!
   ...
   landfrac, lat, long, errmsg, errflg,iulog)
...
! RBN mean in-cloud vertical velocity for use in generating grid-box specific timescale.
   real(kind_phys), intent(in) :: wm_ic(ncol) ! in-cloud vertical velocity  
...
!RBN calculate timescale based on convective height and mean in-cloud velocity.
   ...
   if (ltau_dynamic) then
     do i = il1g,il2g
        tau_wz(i) = 0._kind_phys
        if (wm_ic(i) > 0._kind_phys) then
           tau_wz(i) = (z(i,lel(i))-z(i,mx(i)))/wm_ic(i)
        else
           tau_wz(i) =  10000._kind_phys
        end if
