...
CONTAINS
   ...
   !***************************************************************************
   !
   subroutine get_gz_given_dp_Tv_Rdry_1hd(dp, T_v, R_dry, phis, ptop, gz, pmid, qhe_term)
     use dycore, only: dycore_is
     real(r8), intent(in)  :: dp   (:,:)                       ! pressure level thickness
     ...
     real(r8), intent(out) :: gz(:,:)                          ! geopotential
     real(r8), optional, intent(out) :: pmid(:,:)              ! mid-level pressure
     real(r8), optional, intent(in)  :: qhe_term(:,:)                    ! Quasi-hydrostatic shallow term. 
     ...
     real(r8), dimension(SIZE(dp, 1), SIZE(dp, 2))      :: qhe_term_local
     integer :: kdx

     if (present(qhe_term)) then
       qhe_term_local(:,:) = qhe_term(:,:)
     else
       qhe_term_local = 0
     end if
     ...
     if (dycore_is('LR') .or. dycore_is('FV3')) then
       ...
       do kdx = SIZE(dp,2), 1, -1
         Rdry_tv(:) = (1 + qhe_term_local(:,kdx)) * R_dry(:,kdx) * T_v(:, kdx)
   ...
   subroutine get_gz_given_dp_Tv_Rdry_2hd(dp, T_v, R_dry, phis, ptop, gz, pmid,qhe_term)
     ...
     do jdx = 1, SIZE(dp, 2)
       if (present(pmid)) then
         call get_gz(dp(:, jdx, :), T_v(:, jdx, :), R_dry(:, jdx, :), phis(:, jdx), &
              ptop, gz(:, jdx, :), pmid=pmid(:, jdx, :),qhe_term=qhe_term(:,jdx,:))
       else
