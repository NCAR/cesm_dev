...
contains
  ...
  subroutine advance_hypervis_dp(edge3,elem,fvm,hybrid,deriv,nt,qn0,nets,nete,dt2,eta_ave_w,inv_cp_full,hvcoord)
   ...
   subroutine compute_and_apply_rhs(np1,nm1,n0,dt2,elem,hvcoord,hybrid,&
     ...
     use physconst,       only: tref,cpair,rga,lapse_rate,gravit,rearth
    ...
    ! local
     ...
     real (kind=r8), dimension(np,np,nlev)                         :: qhe_term
...
!JMD  call t_barrierf('sync_compute_and_apply_rhs', hybrid%par%comm)
     ...
     do ie=nets,nete
       ...
       do k=1,nlev
         do j=1,np
           do i=1,np
             qhe_term(i,j,k) = elem(ie)%fcorcos(i,j) * elem(ie)%state%v(i,j,1,k,n0) 
           end do
         end do
       end do
       call get_gz(dp_full, T_v, R_dry, elem(ie)%state%phis, ptop, phi, pmid=p_full,qhe_term=qhe_term)
       do k=1,nlev
        ...
        !DIR_VECTOR_ALIGNED
         ...
         do j=1,np
           do i=1,np
             ...
             vtens1(i,j,k) =   &
                  + v2*(elem(ie)%fcor(i,j) * (1 + 2 * phi(i,j,k)/(gravit* rearth)) + vort(i,j,k))        &
                  - vtemp(i,j,1) - glnps1
             vtens1(i,j,k) = vtens1(i,j,k) + (omega_full(i,j,k) * density_inv(i,j) / gravit) * elem(ie)%fcorcos(i,j)


             vtens2(i,j,k) =   &
                  - v1*(elem(ie)%fcor(i,j) * (1 + 2 * phi(i,j,k)/(gravit * rearth)) + vort(i,j,k))        &
                  - vtemp(i,j,2) - glnps2
