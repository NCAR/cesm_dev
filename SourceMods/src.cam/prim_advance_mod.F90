...
!$omp parallel do num_threads(vert_num_threads) private(k)
  ...
  subroutine hypervis_Qdp(elem, deriv, hybrid, tl_f, tl_qdp, dt_fvm, nets, nete)
    ...
    real(r8) :: dt_sub
    integer  :: ie, k, isub
    ! hypervis_subcycle_cslam_q is auto-set in print_cfl from the del4 stability
    ! bound (ceiling(dt_remap/dt_max_hypervis_cslam_q)); do NOT hardcode it here.
    ! With nu_q_cslam=0.5*nu_p it resolves to 1 at ne120 (single iteration).
