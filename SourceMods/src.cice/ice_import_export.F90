module ice_import_export
  ...
  use ice_domain_size    , only : nfreq, nfsd, max_blocks
...
!==============================================================================
  ...
  subroutine ice_advertise_fields(gcomp, importState, exportState, flds_scalar_name, rc)
    ...
    if (flds_wave) then
       ! This is a copy of ifrac to be sent to wav after applying a mask
       call fldlist_add(fldsFrIce_num, fldsFrIce, 'Si_ifrac_wav' )
  ...
  subroutine ice_export( exportState, rc )
    ...
    real    (kind=dbl_kind) :: ailohi_wav(nx_block,ny_block,max_blocks) ! fractional ice area applied to wav
    ...
    if(.not. allocated(LSMask)) then
       ...
       filename = "/glade/work/gmarques/cesm/tx2_3/mle_frontal_length_scale/mle-cr-mask-v5_20251009.nc"
    ...
    do iblk = 1, nblocks
       ...
       do j = jlo,jhi
          do i = ilo,ihi
             ailohi_wav(i,j,iblk) = ailohi(i, j, iblk)*LSMask(i, j, iblk)
          enddo
       enddo
    enddo
