...
!================================================================================
  ...
  subroutine esmFldsExchange_cesm(gcomp, phase, rc)
    ...
    if (phase == 'advertise') then
       ...
       call addfld_from(compice, 'Si_ifrac_wav')
       call addfld_to(compwav, 'Si_ifrac_wav')
    else
       ...
       if ( fldchk(is_local%wrap%FBexp(compwav)         , 'Si_ifrac_wav', rc=rc) .and. &
            fldchk(is_local%wrap%FBImp(compice,compice ), 'Si_ifrac_wav', rc=rc)) then
             ! By default will be using a custom map - but if one is not available, use a generated bilinear instead
          call addmap_from(compice, 'Si_ifrac_wav', compwav, mapbilnr_nstod, 'one', ice2wav_map)
          call addmrg_to(compwav, 'Si_ifrac_wav', mrg_from=compice, mrg_fld='Si_ifrac_wav', mrg_type='copy')
       end if
    end if
    !----------------------------------------------------------
