...
!wgl
use CVMix_kpp, only : CVMix_kpp_compute_ER_Richardson
...
type, public :: KPP_CS ; private
  ...
  integer :: id_ERdepth  = -1
  integer :: id_StokesXI = -1
  integer :: id_ER_Diag  = -1, id_BEdE_ER  = -1
  integer :: id_Lam2     = -1
  integer :: id_BEdE     = -1, id_ER_Ri    = -1
  !>@}

! KPP_OBLdepth  StokesXI  Lam2  ER_Diag
!                        StokesParXI BEdE_Par Ptke_Par
  ! Diagnostics arrays
  ...
  real, allocatable, dimension(:,:)   :: BEdE_Par    !< Enrtainment Rule's Parameterized BEdE [ m3 s-3 ]
  real, allocatable, dimension(:,:)   :: Ptke_Par    !< Parameterized total BL TKE Production [ m3 s-3 ]
  ...
  real, allocatable, dimension(:,:)   :: ERdepth   !< ER boundary layer depth
  real, allocatable, dimension(:,:,:) :: BEdE      !< calculated Entrainment Flux times Depth [ m3 s-3 ]
  real, allocatable, dimension(:,:,:) :: BEdE_ER   !< calculated Entrainment Flux times Depth [ m3 s-3 ]
  real, allocatable, dimension(:,:,:) :: ER_Ri     !< BEdE - BEdE_Par   [ m3 s-3 ]
...
!/ 2. Options related to enhancing the unresolved Vt2/entrainment in Rib
  ...
  ! Redgister diagnostics  KPP_OBLdepth
  CS%diag => diag
  ...
  CS%id_ERdepth  = register_diag_field('ocean_model', 'KPP_ERdepth', diag%axesT1, Time, &
      'Entrainment Rule Boundary Layer depth', 'm', conversion=US%Z_to_m)
  CS%id_ER_Diag  = register_diag_field('ocean_model', 'ER_Diag',  diag%axesT1, Time, &
      'Entrainment Rule', 'm3 s-3')      
! CS%id_ER_Diag  = register_diag_field('ocean_model', 'P_TKE' , diag%axesT1, Time, &
!     'Total BL TKE Production', 'm3 s-3')
  CS%id_Lam2     = register_diag_field('ocean_model', 'Lam2',  diag%axesT1, Time, &
      'Ustk0_ustar', 'nondim')
  CS%id_BEdE = register_diag_field('ocean_model', 'KPP_BEdE', diag%axesT1, Time, &
      'Product of Entrainment Flux and Depth', &
      'm3/s3', conversion=US%L_T_to_m_s**3)
  CS%id_BEdE_ER  = register_diag_field('ocean_model', 'BEdE_ER', diag%axesTL, Time, &
      'Entrainment Rule BEdE_ER',  'm3 s-3', conversion=US%L_T_to_m_s**3)
  CS%id_ER_Ri = register_diag_field('ocean_model', 'KPP_ER_Ri', diag%axesTL, Time, &
      'BEdE - BEdE_ER', 'm3/s3', conversion=US%L_T_to_m_s**3)
  endif
  ...
  allocate( CS%BEdE_Par( SZI_(G), SZJ_(G) ), source=0. )
  allocate( CS%Ptke_Par( SZI_(G), SZJ_(G) ), source=0. )
  ...
  if (CS%id_ERdepth > 0)  allocate( CS%ERdepth( SZI_(G), SZJ_(G) ), source=0. )
  if (CS%id_BEdE_ER > 0)  allocate( CS%BEdE_ER( SZI_(G), SZJ_(G), SZK_(GV) ), source=0. )
  if (CS%id_BEdE > 0)     allocate( CS%BEdE( SZI_(G), SZJ_(G), SZK_(GV) ), source=0. )
  if (CS%id_ER_Ri > 0)    allocate( CS%ER_Ri( SZI_(G), SZJ_(G), SZK_(GV) ), source=0. )
...
subroutine KPP_compute_BLD(CS, G, GV, US, h, Temp, Salt, u, v, tv, uStar, buoyFlux, Waves, lamult)
  ...
  real :: uSb_BL, vSb_BL, uSb_SL, vSb_SL        ! Average Stokes to Boundary Layer and Surface Layer Depths
                                                               ! [L T-1 ~> m s-1]
  real :: StokesXI     ! Stokes similarity parameter [nondim]
  real :: BEdE_ER      ! Entrainment Rule  [ m3 s-3 ]
  real :: P_TKE        ! Total TKE production rate  [ m3 s-3 ]
  ...
  real, dimension( GV%ke )   :: BEdE_1d                   ! Entrainment Flux times depth    [Z^3 T-3 ~> m s-1]
  real, dimension( GV%ke )   :: dEnt_1d                   ! Enrtainment depth               [Z   ~> m ]
  real, dimension( GV%ke )   :: ER_Ri_1d                  ! Entrainment Rule Adjusted Richardson Number
  
  real :: ERdepth, kERdepth ! Entrainmnent Rule Boundary layer depth  CVMix_kpp_compute_OBL_depth(ER_Ri_1d) in MKS units [m]
  ...
  !$OMP                           StokesXI, StokesXI_1d, StokesVt_1d, kbl), surfBuoy_NS, &
  ...
  do j = G%jsc, G%jec
    do i = G%isc, G%iec ; if (G%mask2dT(i,j) > 0.0) then
      ...
      if (CS%StokesMOST) then
        surfBuoy_NS  = 0.0
        if ( buoyFlux(i,j,3) .gt. 0.0) then
          etaDk = alog(buoyFlux(i,j,2)/buoyFlux(i,j,3)) / (z_inter(2)-z_inter(3))
          surfBuoy_NS = buoyFlux(i,j,2) * exp( -etaDk * z_inter(2) ) ! Temporary ~ solar buoyancy
        endif
        ...
        do k=1,GV%ke
          ...
          surfBuoyFlux2(k) = buoy_scale * &
                          (buoyFlux(i,j,1) - 0.5*(buoyFlux(i,j,k)+buoyFlux(i,j,k+1)) )
        enddo

        ! Initialize StokesMOST surface quantities 
        surfBuoyFlux2(1) = buoy_scale * (buoyFlux(i,j,1) - 0.5*(surfBuoy_NS+buoyFlux(i,j,2)) )
        surfBuoy_NS = buoy_scale * (buoyFlux(i,j,1) - surfBuoy_NS)  ! Total-solar=non-solar
        call Compute_StokesDrift(i, j, h(i,j,1), iFaceHeight(1), &
              0.5*h(i,j,1), iFaceHeight(1), -h(i,j,1),  &
              uS_Hi(1), vS_Hi(1), uS_H(1), vS_H(1), uS_SL, vS_SL, &
              uSbar_H(1), vSbar_H(1), uSb_BL, vSb_BL, uSb_SL, vSb_SL, waves)
      endif
      ...
      do k=1,GV%ke
        ...
        if (CS%StokesMOST) then
          surfBuoyFlux = surfBuoyFlux2(k)                    ! to cell centers
          ...
          call Compute_StokesDrift(i,j, iFaceHeight(k),iFaceHeight(k+1), &
              ...
              uSbar_H(k), vSbar_H(k), uSb_BL, vSb_BL, uSb_SL, vSb_SL, waves) 

          call cvmix_kpp_compute_StokesXi( iFaceHeight, CellHeight, k, ksfc ,SLdepth_0d, surfBuoyFlux, &
               surfFricVel,waves%omega_w2x(i,j), uE_H, vE_H, uS_Hi, vS_Hi, uSbar_H, vSbar_H, & 
               uS_SL, vS_SL, uSb_BL, vSb_BL, uSb_SL, vSb_SL, &
               StokesXI,BEdE_ER,P_TKE,CVMix_kpp_params_user=CS%KPP_params )
    ...
    if (CS%StokesMOST) then
      ...
      call CVMix_kpp_compute_ER_Richardson(          &
                 z_inter,                            & ! (in) Interface heights <= 0  [m]       
                     N2_1d,                          & ! (in) Column of Buoyancy Gradients at interfaces
                         deltaRho,                   & ! (in) Column of Local density difference from surface at centers [kg m-3]
                 OBL_depth,                          & ! (in) Array of assumed OBL depths [m]
                      ...
                      BEdE_1d,                       & ! (out) Entrainment Buoyancy Flux times depth given OBL_depth
                      dEnt_1d,                       & ! (out) Entrainment Depth of BEent
                       ER_Ri_1d,                     & ! (out) Entrainment Rule Adjusted to Ri
                       ERdepth,                      & ! (out) Enntrainment Rule depth
                 CVMix_kpp_params_user=CS%KPP_params ) ! KPP parameters
      kERdepth = cvmix_kpp_compute_kOBL_depth(z_inter, z_cell, ERdepth)  

!
!    BEdE_ER_1d,BEdE_1d,dEnt_1d,ER_Ri_1d   ,  KPP_ER_depth  kER  
!    BEnt(1) = 0 dEnt(1)=0 for kbl=2,nlev  each OBL_depth(kbl) and iterface (i=kbl,2,-1)  CVMix_kpp_compute_ER_Richardson has a 
!    call cvmix_kpp_compute_turbulent_scales[_0d] (  &
!              sigma(i) = -z_inter(i)/OBL_depth(kbl) ,
!              OBL_depth(kbl) , 
!              surfBuoy(kbl)  ,
!              uStar,           
!              StokesXI(kbl)  ,
!              w_s=ws_i      ,                &  ! (out) turbulent scalar velocity scale at interface i
!              CVmix_kpp_params_user)

!   sigma(kbl+1) = 1.0                             ; BE(kbl+1) = 0
!   sigma(kbl+2) = 2.0                             ; BE(kbl+2) = 0

!   until first peak above is found : BE(i+1) > BE(i+2) AND BE(i+1) > BE(i)  AND BE(i+1) > 0
!   BE(i) = Gsig_i * ( ws_i * OBL_depth(kbl) * Nsq(i) - Cemp_Rs * 0.5 * (abs(surfBuoy(kbl)) - surfBuoy(kbl))
!   is found and interpolation of [BE(i+2),BE(i+1),BE(i)] at [sigma(i+2),sigma(i+1),sigma(i)] gives BEnt(kbl) at a depth dEnt(kbl) 
!   Cemp_Rs=4.7 is an empirical constant
!   cvmix_kpp_composite_Gshape(sigma_i , Gat1, Gsig_i, dGdsig)  : Gat1=0                           
!
!   BEdE(kbl)  = BEnt(kbl)*dEnt(kbl)
!   ER_Ri(kbl) = ( Fdelrho * BEdE(kbl) - BEdE_ER(kbl) + Ricr )  : Fdelrho = 1.0  

!   To find depth KPP_ER_depth, where ER_Ri = Ricr              : Fdelrho*BEdE=BEdE_ER   
!    call CVMix_kpp_compute_OBL_depth( &
!           ER_Ri_1d,               & ! (in) Entrainment Rule Richardson number
!           z_inter,                & ! (in) Height of interfaces [m]
!           KPP_ER_depth,           & ! kER (out) Entrainment Rule OBL depth [m]
!           kER,                    & ! (out) level (+fraction) of OBL extent
!           zt_cntr=z_cell,         & ! (in) Height of cell centers [m]
!           surf_fric=surfFricVel,  & ! (in) Turbulent friction velocity at surface [m s-1]
!           surf_buoy=surfBuoyFlux2,& ! (in) Buoyancy flux at surface [m2 s-3]
!           Coriolis=Coriolis,      & ! (in) Coriolis parameter [s-1]
...
!           CVMix_kpp_params_user=CS%KPP_params ) ! KPP parameters
     ...
     if ( ERdepth > -z_inter(2) ) then
       CS%OBLdepth(i,j) = US%m_to_Z * ERdepth  
       CS%kOBL(i,j)     = kERdepth
     endif    
!
      kbl = int(CS%kOBL(i,j))
...
!  DIAGNOSTICS
      call Compute_StokesDrift(i,j, iFaceHeight(kbl), iFaceHeight(kbl+1), &
              ...
              uSbar_H(kbl), vSbar_H(kbl), uSb_BL, vSb_BL, uSb_SL, vSb_SL, waves)

      call cvmix_kpp_compute_StokesXi(iFaceHeight, CellHeight, kbl, ksfc ,SLdepth_0d, surfBuoyFlux, &
              surfFricVel,waves%omega_w2x(i,j), uE_H, vE_H, uS_Hi, vS_Hi, uSbar_H, vSbar_H, &
              uS_SL, vS_SL, uSb_BL, vSb_BL, uSb_SL, vSb_SL, &
              StokesXI,BEdE_ER,P_TKE,CVMix_kpp_params_user=CS%KPP_params )

      CS%StokesParXI(i,j) = StokesXI
      CS%BEdE_Par(i,j)    = BEdE_ER
      CS%Ptke_Par(i,j)    = StokesXI  
      CS%Lam2(i,j)        = sqrt(US_Hi(1)**2+VS_Hi(1)**2) / MAX(surfFricVel,0.0002)

      if (CS%id_ERdepth > 0) CS%ERdepth(i,j) = ERdepth

      if (CS%id_BulkRi > 0)   CS%BulkRi(i,j,:) = BulkRi_1d(:)
      if (CS%id_BEdE   > 0)   CS%BEdE(i,j,:) = BEdE_1d(:)
      if (CS%id_BEdE_ER > 0)  CS%BEdE_ER(i,j,:) = BEdE_ER_1d(:)
      if (CS%id_ER_Ri  > 0)   CS%ER_Ri(i,j,:) = ER_Ri_1d(:)
      


    else                         !.not Stokes_MOST
      CS%StokesParXI(i,j) = 1.0
      CS%BEdE_Par(i,j)    = 10.0       
      CS%Ptke_Par(i,j)    = 100.0   
      CS%Lam2(i,j)        = sqrt(US_Hi(1)**2+VS_Hi(1)**2) / MAX(surfFricVel,0.0002)
  ...
  if (CS%StokesMOST) then
    ...
    if (CS%id_ER_Diag  > 0) call post_data(CS%id_ER_Diag,  CS%Ptke_Par,       CS%diag)
!   if (CS%id_BEdE_ER  > 0) call post_data(CS%id_BEdE_ER,  CS%BEdE_Par,       CS%diag)
    ...
    if (CS%id_BEdE     > 0) call post_data(CS%id_BEdE   ,  CS%BEdE,           CS%diag)
    if (CS%id_ER_Ri    > 0) call post_data(CS%id_ER_Ri  ,  CS%ER_Ri,          CS%diag)
...
!> Components at 1) zbot < ztop <= 0 ; 2) zBL=Boundary Layer Height ; 3) zSL=Surface Layer Height
!! average components from 1) ztop to zbot ; 2) ztop to zBL ; 3) zSLtop to zSL

subroutine Compute_StokesDrift(i ,j, ztop, zbot, zBL, zSLtop, zSL, uS_i, vS_i, uS_k, vS_k, uS_SL, vS_SL, &
               uSbar, vSbar, uSb_BL, vSb_BL, uSb_SL, vSb_SL, waves)

  type(wave_parameters_CS), pointer  :: waves           !< Wave CS for Langmuir turbulence
  real,                intent(in)    :: ztop, zbot, zBL !< boundary layer cell and height
  real,                intent(in)    :: zSLtop , zSL    !< surface layer cell top and depth
  real,                intent(inout) :: uS_i , vS_i     !< Stokes velocity at zbot interface
  real,                intent(inout) :: uS_k , vS_k     !< Stokes velocity at zbl                 
  real,                intent(inout) :: uS_SL, vS_SL    !< Stokes at zSL             
  real,                intent(inout) :: uSbar, vSbar    !< mean Stokes (ztop to zbot)
  real,                intent(inout) :: uSb_BL, vSb_BL  !< mean Stokes (ztop to zBL)
  real,                intent(inout) :: uSb_SL, vSb_SL  !< mean Stokes (zSLtop to zSL)
  ...
  do b  = 1, waves%NumBands
    ...
    fexp  =  exp(2. * WaveNum * ztop) - exp(2. * WaveNum * zBL  )
    uSb_BL=  uSb_BL + 0.5 * waves%Ustk_Hb(i,j,b) * fexp / WaveNum
    vSb_BL=  vSb_BL + 0.5 * waves%Vstk_Hb(i,j,b) * fexp / WaveNum
  ...
  uSb_BL = uSb_BL / (ztop-zBL)
  vSb_BL = vSb_BL / (ztop-zBL)
