...
!BOC
    ...
    if (present(N_iface)) then
      ...
      do kt=1,nlev
        N_cntr(kt) = N_iface(kt+1)  !Ndn
!wgl    N_cntr(kt) = N_iface(kt)    !Nup
      end do
    else
      if (present(Nsqr_iface)) then
        ...
        do kt=1,nlev
!wgl      N_cntr(kt)=sqrt(max(Nsqr_iface(kt+1),cvmix_zero))
!N2a      N_cntr(kt)= sqrt( 0.5_cvmix_r8 * ( max(Nsqr_iface(kt),cvmix_zero) + max(Nsqr_iface(kt+1),cvmix_zero) ) )
          N_cntr(kt)=sqrt(max(Nsqr_iface(kt),cvmix_zero)) ! N2up
        end do
      else
...
!   if (present(N_iface)) then
      ...
      Cv = 2.4_cvmix_r8 ! call this

      do kt=1,nlev
        xbeta = MAX( cvmix_zero , MIN( (300._cvmix_r8-100._cvmix_r8) , (-zt_cntr(kt) - 100._cvmix_r8) ) )
        ybeta = cvmix_one - ( xbeta / (300._cvmix_r8-100._cvmix_r8) )
        ybeta = MAX( ybeta*ybeta , 0.01_cvmix_r8 )
        beta  = ybeta * 0.2_cvmix_r8
!C      beta  =  MAX(0.005_cvmix_r8, 0.2_cvmix_r8*80.0_cvmix_r8/MAX( 80.0_cvmix_r8,abs(zt_cntr(kt))))
