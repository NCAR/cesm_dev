...
subroutine convert_IOB_to_fluxes(IOB, fluxes, index_bounds, Time, valid_time, G, US, CS, &
  ...
  do j=js,je ; do i=is,ie
    ...
    if (associated(IOB%lrunoff)) then
      ...
      if( (G%geolatT(i,j) >  45. ) .and. (G%geolatT(i,j) <  75. ) .and.  &
        (G%geolonT(i,j) > -70. ) .and. (G%geolonT(i,j) < -20. )) then

        fluxes%lrunoff(i,j) = 0.5 * kg_m2_s_conversion * IOB%lrunoff(i-i0,j-j0) * G%mask2dT(i,j)
      else
    ...
    if (associated(IOB%frunoff)) then
      ...
      if( (G%geolatT(i,j) >  45. ) .and. (G%geolatT(i,j) <  75. ) .and.  &
        ...
        fluxes%frunoff(i,j) = 0.5 * kg_m2_s_conversion * IOB%frunoff(i-i0,j-j0) * G%mask2dT(i,j)
      else
