...
type, public :: hbd_CS ; private
  ...
  logical :: mask_hbd        !< If true, read a file (hbd_mask) containing a mask to control where
                             !! HBD is applied. This can be use to shut off HBD in the Lab Sea.
  ...
  real, pointer, dimension(:,:) :: hbd_mask => NULL()   !< mask for HBD                             [nondim]
  ! HBD dynamic grids
...
logical function hor_bnd_diffusion_init(Time, G, GV, US, param_file, diag, diabatic_CSp, CS)
  ...
  character(len=48)  :: flnam   ! Used to read mask
  ...
  call get_param(param_file, mdl, "HBD_MASK", CS%mask_hbd, &
                 "If true, read a file (hbd_mask) containing a mask to control where"//&
                 "HBD is applied. This can be use to shut off HBD in the Lab Sea.", default=.false.)
  call safe_alloc_ptr(CS%hbd_mask,isd,ied,jsd,jed); CS%hbd_mask(:,:) = 1.0
  if (CS%mask_hbd) then ! read a 2-d file containing a mask for HBD
    flnam = trim(CS%inputdir) // 'hbd_mask.nc'
    call MOM_read_data(flnam, 'mask', CS%hbd_mask, G%domain, timelevel=1)
  endif
  ...
  do m = 1,Reg%ntr
      ...
      do i=G%isc-1,G%iec
        if (G%mask2dCu(I,j)>0.) then
           call fluxes_layer_method(SURFACE, GV%ke, hbl(I,j), hbl(I+1,j),  &
            ...
            CS%hbd_grd_u(I,j,:), CS)
           uFlx(I,j,:) = uFlx(I,j,:) * MIN(CS%hbd_mask(I,j), CS%hbd_mask(I+1,j))
        endif
      enddo
    enddo
    do J=G%jsc-1,G%jec
      do i=G%isc,G%iec
        if (G%mask2dCv(i,J)>0.) then
          call fluxes_layer_method(SURFACE, GV%ke, hbl(i,J), hbl(i,J+1),  &
            ...
            CS%hbd_grd_v(i,J,:), CS)
          vFlx(i,J,:) = vFlx(i,J,:) * MIN(CS%hbd_mask(i,J), CS%hbd_mask(i,J+1))
        endif
      enddo
    enddo
    ...
    do k=1,GV%ke ; do j=G%jsc,G%jec ; do i=G%isc,G%iec
      if (G%mask2dT(i,j)>0.) then
        tracer%t(i,j,k) = tracer%t(i,j,k) + (((uFlx(I-1,j,k)-uFlx(I,j,k))) + &
                          ((vFlx(i,J-1,k)-vFlx(i,J,k) )))* G%IareaT(i,j) / ( h(i,j,k) + GV%H_subroundoff)
