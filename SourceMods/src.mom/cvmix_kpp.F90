...
!wgl
  public :: cvmix_kpp_compute_ER_Richardson
...
! !DESCRIPTION:
!  Compute the Stokes similarity parameter, StokesXI, Entrainment Rule, BEdE,
!  AND total BL tke production from integrated TKE production terms as 
!  parameterized in Large et al., 2021 (doi:10.1175/JPO-D-20-0308.1) 
!\\
!\\

  subroutine cvmix_kpp_compute_StokesXi (zi, zk, kBL, kSL, SLDepth, surf_buoy_force,   &
           surf_fric_vel, omega_w2x, uE, vE, uS, vS, uSbar, vSbar,                     &
           uS_SL, vS_SL, uSb_BL, vSb_BL, uSb_SL, vSb_SL,                               &
           StokesXI,BEdE_ER,P_TKE,CVmix_kpp_params_user)

! !INPUT PARAMETERS:
    real(cvmix_r8), dimension(:), intent(in) :: zi, zk          !< Cell interface and center heights < 0
    integer,        intent(in) :: kBL                           !< cell index of Boundary Layer Depth
    ...
    real(cvmix_r8), intent(in) :: uSb_BL, vSb_BL                !< Average Stokes drift cell top to BLDepth
...
! !OUTPUT PARAMETERS:
    ...
    real(cvmix_r8), intent(inout) :: P_TKE                      ! Integrated rate of TKE Production of  [m3 s-3]
...
!   PB     = PBfact * MAX( -surf_buoy_force * BLdepth ,  cvmix_zero )
!   PB     = -surf_buoy_force * BLdepth * PBfact                             
...
!   PS     = PS +  Pinc
    ...
    ! Compute Stokes similarity parameter,   Entrainment Rule, and TKE prodction Rate
...
!   BEdE_ER   = 0.8_cvmix_r8 * MAX( Cu * PU + Cs * PS + Cb * PB , cvmix_zero )
    BEdE_ER   =                MAX( Cu * PU + Cs * PS + Cb * PB , cvmix_zero )

! Return to top of surface layer cell
    PS     = PS -  MAX( Pinc , cvmix_zero )
!   PS     = PS - Pinc
    Pinc   = 0.5_cvmix_r8 * (tauEbot + tauEtop) * delH  ! May 18/25 BugFix
    PU     = PU - MAX( Pinc , cvmix_zero ) 
!   PU     = PU - Pinc

! Integrate  from top of surface layer cell to top of boundary layer cell
    do ktmp = kSL, kBL-1
      delU     = uE(ktmp) - uE(ktmp+1)
      delV     = vE(ktmp) - vE(ktmp+1)
      Omega_E2x= atan2( delV  , delU )
      ...
      sigbot  = dbot / BLdepth
      Gbot    = cvmix_kpp_composite_shape(sigbot)
      TauMAG  = ustar * ustar * ( cvmix_one - sigbot )
      tauCG   = CempCGm * Gbot *  (taux0 * cosOmega - tauy0 * sinOmega)
      ! tauDG = sqrt( TauMAG**2 - tauCG**2 ) ! G
      tauDG   = TauMAG                       ! E
      tauxbot = tauDG * cosOmega  -  tauCG * sinOmega
...
! Integrate from top of boundary layer cell to boundary layer depth
    delH = BLdepth - dtop     ! BLDepth + zi(kBL)
    Pinc = 0.5_cvmix_r8 * tauEbot * delH
    PU   = PU + MAX( Pinc , cvmix_zero )
!   PU   = PU + Pinc
    
    Pinc = tauxtop*uS(ktmp) + tauytop*vS(ktmp) 
    Pinc = Pinc - (tauxtop * uSb_BL)  - (tauytop * vSb_BL)             
    PS   = PS +  MAX( Pinc , cvmix_zero )
!   PS   = PS + Pinc
...
! !DESCRIPTION:
! Find Entrainment Flux and Depth                                          

  subroutine cvmix_kpp_compute_ER_Richardson( z_inter,Nsq,deltaRho,OBL_depth, & 
           uStar,Bsfc,surfBuoy,StokesXI,BEdE_ER,BEdE,dEnt,ER_Ri,ERdepth,  &
           CVMix_kpp_params_user)
...
! !INPUT PARAMETERS:
    ...
    real(cvmix_r8), dimension(:), intent(in) :: &
                 OBL_depth,                     & !  Array of assumed OBL depths [m]
    ...
    real(cvmix_r8),  intent(inout) :: Bsfc        ! non-solar surface buoyancy flyx boundary condition
    real(cvmix_r8),  intent(out)   :: ERdepth     ! ER Boundary Layer Depth [m]
  type(cvmix_kpp_params_type), intent(in), optional, target ::             & 
                  CVmix_kpp_params_user

! !OUTPUT PARAMETERS:
  real(cvmix_r8), dimension(size(OBL_depth)), intent(inout) ::         &
                  BEdE,                              & ! (inout) Entrainment Buoyancy Flux times depth given OBL_depth
                  dEnt,                              & ! (inout) Entrainment Depth of BEent
                 ER_Ri                                 ! (inout) Entrainment Rule Adjusted to Ri
...
!BOC
  ...
  ! Local variables
  integer ::  iz, nlev , kbl
  real(cvmix_r8), dimension(size(z_inter)+1) ::  sigma, Bflux 
  real(cvmix_r8)   ::  ws_i , Cemp_Rs, Gat1 ,Gsig_i, dGdsig , Fdelrho, BEnt, Bnonlocal, sigE, eta

  nlev = size(OBL_depth)
  Cemp_Rs = 4.7_cvmix_r8       ! 2.1_cvmix_r8-real(200,cvmix_r8)
  Gat1    = cvmix_zero
  Fdelrho = cvmix_one
  ...
  Bflux(1) = Bsfc    ! non-solar surface buoyancy boundary condition valid for all kbl   
  do kbl = 2, nlev    
    dEnt(kbl) =   -cvmix_one
    BEdE(kbl) =  cvmix_zero
    ER_Ri(kbl)= -cvmix_one 
    ...
    do iz = kbl,1,-1 
      ...
      if (iz .gt. 1) then
        ...
        call cvmix_kpp_composite_Gshape(sigma(iz) , Gat1, Gsig_i, dGdsig)  
        Bflux(iz) = Gsig_i * (ws_i * OBL_depth(kbl) * Nsq(iz) - Bnonlocal)
      endif
      ...
      if ( (Bflux(iz+1) > Bflux(iz+2)) .and. (Bflux(iz+1) .ge. Bflux(iz)) .and. (Bflux(iz+1) > cvmix_zero) ) then
        ...
        dEnt(kbl) = sigE * OBL_depth(kbl)
        Fdelrho   = cvmix_one
        BEdE(kbl)  = BEnt*dEnt(kbl)  
!       if (Bsfc < cvmix_zero  )  ER_Ri(kbl) = ( BEdE(kbl) - BEdE_ER(kbl) + cvmix_one )
        ER_Ri(kbl) = ( BEdE(kbl) - BEdE_ER(kbl) + cvmix_one )
      exit
      endif  
    enddo
!          if (ER_Ri(kbl) ge 0) then an ER_Depth is realizable; compute?  exit ?
    if ( (BEdE(kbl) > BEdE_ER(kbl)) .and. (Bsfc < cvmix_zero) ) then
!   if ( (BEdE(kbl) > BEdE_ER(kbl)) ) then
      ERdepth = OBL_depth(kbl-1) - (ER_Ri(kbl-1)-cvmix_one) * (OBL_depth(kbl)-OBL_depth(kbl-1)) / (ER_Ri(kbl)-ER_Ri(kbl-1))
    exit 
    endif
  enddo
!EOC

  end subroutine cvmix_kpp_compute_ER_Richardson
...
!   print*, "FIT", i,size(x),size(y),a,b,c,x0,y0
