...
bioms:   do f = 1, fn
            ...
            !KO
               ...
               ! and high leaf temperature. The 0.1_r8 value could be an adjustable
               ! parameter.
               if(elai(p) .lt. 0.1_r8) then 
                  sa_leaf(p) = sa_leaf(p) + esai(p)
                  ! This is an additional option for cases where esai is also small and
                  ! thus doesn't help much with small leaf conductance.  Although I tested
                  ! this and it didn't seem to help much. Might need to increase the 
                  ! minimum value.
                  !KOsa_leaf(p) = max(sa_leaf(p) + esai(p),0.1_r8)
               endif
            !KO
            endif
